...
public class BlurLayout extends FrameLayout {
    ...
    public BlurLayout(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.BlurLayoutt,
                0, 0);

        try {
            mDownscaleFactor = a.getFloat(R.styleable.BlurLayoutt_blk_downscaleFactor, DEFAULT_DOWNSCALE_FACTOR);
            mBlurRadius = a.getInteger(R.styleable.BlurLayoutt_blk_blurRadius, DEFAULT_BLUR_RADIUS);
            mFPS = a.getInteger(R.styleable.BlurLayoutt_blk_fps, DEFAULT_FPS);
            mCornerRadius = a.getDimension(R.styleable.BlurLayoutt_blk_cornerRadius, DEFAULT_CORNER_RADIUS);
            mAlpha = a.getDimension(R.styleable.BlurLayoutt_blk_alpha, DEFAULT_ALPHA);
        } finally {
            a.recycle();
        }
    ...
    private Bitmap blur() {
        ...
        if (mViewLocked) {
            ...
            } catch (com.example.blurkit.BlurKitException e) {
                return null;
